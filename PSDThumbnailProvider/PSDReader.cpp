	...
	HBITMAP* phbmp, WTS_ALPHATYPE* pdwAlpha) 
{
	...
	*/
	...
	//  Convert to Bitmap
	// Geometry geometry = Geometry(maxWidth, maxHeigth);
	...
	MessageBoxW(NULL, L"here2", L"Debug", MB_OK);

	if (!hBitmap) {
		MessageBoxW(NULL, L"!hBitmap", L"Debug", MB_OK);
		return E_FAIL;
	}
	...
	*pdwAlpha = WTSAT_ARGB;
	*phbmp = hBitmap;
	MessageBoxW(NULL, L"Success", L"Debug", MB_OK);
	return S_OK;
}